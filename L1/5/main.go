...
func main() {
	...
	ctx, _ := context.WithTimeout(context.Background(), time.Second*time.Duration(seconds)) //создание контекста с таймаутом
	...
	//cancel()                //(!)IDE пишет предупреждение что нужно использовать функцию завершения контекста, не понял зачем ее использовать второй раз, если таймаут и так завершает контекст
